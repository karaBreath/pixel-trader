...
mode = st.sidebar.radio("เลือกโหมด", ["🔎 ร่อนหาหุ้น", "🔬 ดูรายตัว"])
st.sidebar.markdown("---")
st.sidebar.caption("วิเคราะห์เชิงลึกหลาย AI → พิมพ์ `/analyze TICKER` ในแชต Claude")
...
if mode == "🔎 ร่อนหาหุ้น":
    ...
    if st.button("🚀 เริ่มร่อนหุ้น"):
        ...
        st.markdown(f"### 🏆 อันดับหุ้นเด่น — สไตล์ {style}")
        st.dataframe(df, use_container_width=True, hide_index=True, height=560)
        if rows:
            top = ", ".join(r["ticker"] for r in rows[:5])
            st.success(f"Top 5: {top} — เจาะลึกด้วย /analyze <สัญลักษณ์> ในแชต")

else:  # ดูรายตัว
    ticker = st.text_input("ใส่สัญลักษณ์ (เช่น NVDA, PTT.BK, BTC-USD, GLD)", "NVDA").strip().upper()
    if ticker:
